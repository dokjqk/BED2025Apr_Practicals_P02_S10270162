...
app.put("/books/:id", async (req, res) => {
  ...
  try {
    ...
    const fetchUpdatedBookQuery = `SELECT id, title, author FROM Books WHERE id = @id`;
    const fetchRequest = connection.request();
    fetchRequest.input("id", bookId);
    const updatedBook = await fetchRequest.query(fetchUpdatedBookQuery);

    res.status(200).json(updatedBook.recordset[0]);
  } catch (error) {
